...
Page({
  ...
  getMsg: function(t) {
    ...
    wx.getStorage({
      ...
      fail:function() {
        console.log(t.continued)
        if (t.continued || "") {
          var n = wx.getStorageSync("exam_allfen" + e.data.category.id);
          n && e.setData({
            allfen: 1 * n
          });
          var s = wx.getStorageSync(r + "ids" + e.data.category.id);
          console.log(s)
          e.ind_to_data(s), setTimeout(function () {
            wx.hideLoading();
          }, 1e3);
        } else {
          s = e.data.category.id;
          console.log(s)
          wx.getStorage({
            key: n + "" + s,
            success: function (t) {
  ...
  ind_to_data: function (t) {
    var that = this;
    for (var a = this, s = q.getQuestionsByIds(t), i = 0; i < s.length; i++)
    ...
    n = s, a.setData({
      ...
      key: r + "ids" + that.data.category.id,
      data: t
    }), a.getthree();
  },
    ...
    if (e(), "背题模式" != a.data.textTab){
      ...
      if (console.log(n[a.data.indexInd]), console.log(a.data.questions[a.data.current]),
        ...
        if (c++ , t.currentTarget.dataset.ind == t.currentTarget.dataset.answer) {
          ...
          getApp().setIdsStroage("errorids", a.data.category.id, 1, o[a.data.current].question_id.toString());
  ...
  moreSelectSub: function (t){
    ...
    if (h++ , g == t.currentTarget.dataset.answer.length && downAnswer == t.currentTarget.dataset.answer) {
      ...
      getApp().setIdsStroage("errorids", a.data.category.id, 1, u[a.data.current].question_id.toString());
  ...
  storageTraining_qids: function (t) {
    ...
    e = e + "" + t + "," + wx.setStorage({
      key: "training_qids" + this.data.category.id,
      data: e,
    }), this.setData({
      training_qids: e
    });
    ...
    a.push(t), this.setData({
      ...
      key: "intensify_qis" + this.data.category.id,
      data: a,
    });
  },
  ...
  storageokids: function (t) {
    ...
    e.push(t), this.setData({
      ...
      key: "intensify_okids" + this.data.category.id,
      data: e
    })
  },
  ...
  storagenoids: function (t){
    ...
    e.push(t), this.setData({
      ...
      key: "intensify_noids" + this.data.category.id,
      data: e
    })
  ...
  newUp_exam:function() {
    ...
    var e = {
      ...
      statusAnswer: this.data.nums - this.data.allNum,
      statusScore: this.data.allfen,
      statusPassf: this.data.allfen > this.data.passf ? 1 : 0
    };
    this.setData({
      showStatus: !0,
      statusOptions: e
  ...
  result: function() {
    ...
    wx.setStorage({
      key: "exam_up" + this.data.category.id,
      data: 1,
    }), wx.setStorage({
      key: r + "all" + this.data.category.id,
      data: n
    }), 
